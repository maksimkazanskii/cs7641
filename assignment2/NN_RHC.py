...
for learning_rate in LEARNING_RATE:
    ...
    model = mlrose.NeuralNetwork(hidden_nodes =[64], activation='relu', algorithm='random_hill_climb',
                                 max_iters = 50000, clip_max = 10, bias=False, is_classifier=True, learning_rate=learning_rate,
    ...
    print("max_attempts, Train accuracy, Test accuracy ,time:", learning_rate, train_accuracy, test_accuracy, time1 - time0)
...
np.save("data/NN_RHC_TRAIN.npy", TRAIN_ACCURACY)
np.save("data/NN_RHC_TEST.npy",  TEST_ACCURACY )
np.save("data/NN_RHC_FIT.npy",   FIT_ARR)
np.save("data/NN_RHC_TIME.npy",  TIME)

TRAIN_ACCURACY = np.load('data/NN_RHC_TRAIN.npy')
TEST_ACCURACY  = np.load('data/NN_RHC_TEST.npy')
FIT_ARR        = np.load('data/NN_RHC_FIT.npy')
TIME           = np.load('data/NN_RHC_TIME.npy')
...
fig, ax = plt.subplots()
ax.plot(X0, -FIT_ARR[0], color='steelblue', label="Loss curve (step = 1.0)", linewidth=2.0, linestyle='-')
ax.plot(X1, -FIT_ARR[1], color='red', label="Loss curve (step = 0.5)", linewidth=2.0, linestyle='-')
ax.plot(X2, -FIT_ARR[2], color='black', label="Loss curve (step = 0.1)", linewidth=2.0, linestyle='-')


ax.legend(loc='best', frameon=True)
plt.grid(True, linestyle='--')
plt.title('RHC, Loss vs Number of iteration.')
plt.ylabel('Loss')
plt.xlabel('Iterations')
plt.savefig('plots/NN_RHC2.png')
