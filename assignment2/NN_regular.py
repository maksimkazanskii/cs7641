...
LEARNING_RATE = [0.000001,0.0000033,0.00001, 0.000033,0.0001,0.00033, 0.001]
...
for lr in LEARNING_RATE:
    ...
    history.append(nn_model_GD.fit(x_train_cv, y_train_cv))
    time1 = time.time()
    y_train_pred = nn_model_GD.predict(x_train_cv)
    train_accuracy = accuracy_score(y_train_cv, y_train_pred)
    TRAIN_ACCURACY.append(train_accuracy)
    y_test_pred = nn_model_GD.predict(x_test_cv)
    test_accuracy = accuracy_score(y_test_pred, y_test_cv)
    TEST_ACCURACY.append(test_accuracy)
    print("Learning rate, Train accuracy, Test accuracy ,time:", lr, train_accuracy, test_accuracy, time1 - time0)
...
np.save("data/NN_GD_TRAIN.npy", TRAIN_ACCURACY)
np.save("data/NN_GD_TEST.npy",  TEST_ACCURACY )
np.save("data/NN_GD_FIT.npy",   FIT_ARR)
np.save("data/NN_GD_TIME.npy",  TIME)

TRAIN_ACCURACY = np.load('data/NN_GD_TRAIN.npy')
TEST_ACCURACY  = np.load('data/NN_GD_TEST.npy')
FIT_ARR        = np.load('data/NN_GD_FIT.npy')
TIME           = np.load('data/NN_GD_TIME.npy')

X0 = np.arange(0,len(FIT_ARR[1]),1)
X1 = np.arange(0,len(FIT_ARR[3]),1)
X2 = np.arange(0,len(FIT_ARR[6]),1)
fig, ax = plt.subplots()
ax.plot(X0, -FIT_ARR[1], color='steelblue', label="Loss curve (lr = 3.3*10^-6)", linewidth=2.0, linestyle='-')
ax.plot(X1, -FIT_ARR[3], color='red', label="Loss curve  (lr = 3.3*10^-5)", linewidth=2.0, linestyle='-')
ax.plot(X2, -FIT_ARR[6], color='black', label="Loss curve (lr = 10^-3)", linewidth=2.0, linestyle='-')
ax.legend(loc='best', frameon=True)
plt.grid(True, linestyle='--')
plt.ylim((0,5))
plt.xlim((0,5000))
plt.title('Gradient descent, Loss vs Number of iterations.\n Different values learning rates (lr)')
plt.ylabel('Loss')
plt.xlabel('Iterations')
plt.savefig('plots/NN_regular1.png')
...
plt.ylabel(" Accuracy ")
plt.savefig('plots/NN_regular2.png')
